...
class FrameLogin(FramePage):
    def create_widgets(self):
        ...
        tk.Button(self, text="Cancel", width=5, command=lambda: self.app.show_frame(mm.FrameMainMenu)).grid(row=1, column=0, padx=(0, 5), sticky=tk.N+tk.E)
        tk.Button(self, text="Login", width=5, command=self.login).grid(row=1, column=1, padx=(5, 0), sticky=tk.N+tk.W)
